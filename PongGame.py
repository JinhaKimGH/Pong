...
while run:
    screen.fill(BLACK)
    pygame.display.update(pygame.draw.circle(screen, colour, (ball_x, ball_y), radius, radius-1))
    pygame.display.update(pygame.draw.rect(screen, BLUE, (paddle_x, paddle_y, width, height)))
    pygame.display.update(pygame.draw.rect(screen, ORANGE, (second_paddle_x, second_paddle_y, width, height)))
    key = pygame.key.get_pressed()
    pos = pygame.mouse.get_pos()
    text = font.render(winner, True, WHITE, BLACK)
    ...
    if ball_dx == 0 and ball_dy == 0:
        pygame.display.update(screen.blit(start, startRect))
    ...
    if score_player_one == 10 or score_player_two == 10:
        ball_dx = 0
        ball_dy = 0
        pygame.display.update(screen.fill(BLACK))
        pygame.display.update(screen.blit(text, textRect))
        pygame.display.update(screen.blit(write, writeRect))
        pygame.display.update(screen.blit(retry, retryRect))
